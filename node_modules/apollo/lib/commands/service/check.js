...
class ServiceCheck extends Command_1.ProjectCommand {
                ...
                return [
                    {
                        ...
                        task: async (ctx, task) => {
                            ...
                            if (flags.federated) {
                                ...
                                const { errors, warnings, compositionValidationDetails } = await project.engine.checkPartialSchema({
                                    id: configName,
                                    graphVariant: tag,
  ...
  ${errors && errors.map(err => (err && err.message) || "").join("\n")}
        ...
        if (taskOutput.federation) {
            ...
            const warnings = taskOutput.federation.warnings.map(error => reshapeGraphQLErrorToChange(graphqlTypes_1.ChangeSeverity.WARNING, error ? error.message : ""));
            if (errors.length &&
