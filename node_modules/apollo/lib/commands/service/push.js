...
class ServicePush extends Command_1.ProjectCommand {
    async run() {
        ...
        await this.runTasks(({ flags, project, config }) => [
            {
                ...
                task: async () => {
                    ...
                    if (flags.federated) {
                        ...
                        const { compositionConfig, errors, warnings, didUpdateGateway, serviceWasCreated } = await project.engine.uploadAndComposePartialSchema({
                            id: config.name,
        ...
        if ((errors && errors.length) || (warnings && warnings.length)) {
            ...
            const messages = [
                ...
                ...warnings.map(({ message }) => ({
                    type: chalk_1.default.yellow("Warning"),
                    description: message
                }))
            ].filter(x => x !== null);
        ...
        if (result.didUpdateGateway) {
            this.log(`The gateway for the ${result.graphName} graph was updated with a new schema\n`);
        }
        if (result.serviceWasCreated) {
            this.log(`A new service called ${result.service} for the graph ${result.graphName} was created\n`);
        }
        if (!isFederated) {
